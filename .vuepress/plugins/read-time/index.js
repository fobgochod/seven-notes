...
module.exports = (options = {}) => ({
    extendPageData($page) {
        ...
        if (path === '/') {
            ...
            if (timestamp) {
                $page.siteLastUpdated = moment(new Date(timestamp)).format('YYYY-MM-DD HH:mm:ss');
                console.error("编译日期：" + $page.siteLastUpdated)
            } else {
                $page.siteLastUpdated = moment().format('YYYY-MM-DD HH:mm:ss');
                console.error("GIT获取更新时间出错，采用默认时间")
            }

            // 由于README.md会显示在GitHub上，设置frontmatter不好看，改为这里进行
            frontmatter.title = "七的笔记：The Seven Notes"
